...
export default function LogoutButton({ as, children, ...props }: LogoutButtonProps) {
  ...
  switch (as) {
    case "button":
      return (
        <Button onClick={handleLogout} disabled={isPending} {...(props as ButtonProps)}>
          {isPending ? <Spinner size="small" /> : buttonText}
        </Button>
      );
    case "dropdownMenuItem":
      return (
        <DropdownMenuItem onClick={handleLogout} disabled={isPending} {...(props as DropdownMenuItemProps)}>
          {isPending ? <Spinner size="small" /> : buttonText}
        </DropdownMenuItem>
      );
  }
