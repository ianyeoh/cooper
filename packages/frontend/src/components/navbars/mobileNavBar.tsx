      ...
      <Drawer direction="left" open={open} onOpenChange={setOpen}>
        <DrawerTrigger asChild>
          <Button
            ...
            className="inline-flex items-center justify-center whitespace-nowrap rounded-md font-medium transition-colors focus-visible:outline-hidden focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 hover:text-accent-foreground h-9 py-2 mr-2 px-0 text-base hover:bg-transparent focus-visible:bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 md:hidden"
          >
              ...
              <div className="flex flex-col space-y-3">
                {links.map((item) => {
                  switch (item.kind) {
                    case "group":
                      return (
                        <div className="mb-4 flex flex-col gap-2">
                          <h4 className="rounded-md text-md font-semibold">{item.title}</h4>
                          {item.links.map((link) => {
                            return (
                              <Link className="hover:underline" href={link.url} key={link.url}>
                                {link.display}
                              </Link>
                            );
                          })}
                    ...
                    case "link":
                      return (
                        <Link className="hover:underline" href={item.url} key={item.url}>
                          {item.display}
                        </Link>
                      );
                  }
